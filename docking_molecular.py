...
# Tenta importar as bibliotecas principais (Apenas verificação)
try:
    import py3Dmol
...
st.markdown("Plataforma acadêmica para ensino e pesquisa de **Química Medicinal Computacional** e **Planejamento de Fármacos**.")
...
with tab_install:
    ...
    st.markdown("### ⚙️ Instruções de Configuração")
    st.markdown("""
    Esta plataforma requer um ambiente virtual (venv) isolado. Caso o sistema acuse a falta de bibliotecas, abra o seu terminal e execute os comandos abaixo **antes** de rodar o Streamlit:
    """)
    st.code("""
# 1. Crie o ambiente virtual
python3 -m venv docking_env

# 2. Ative o ambiente (Windows: docking_env\\Scripts\\activate)
source docking_env/bin/activate

# 3. Instale todas as dependências científicas
pip install streamlit py3Dmol requests meeko rdkit pubchempy scipy pandas nglview vina numpy openbabel-wheel openmm pdbfixer biopython --break-system-packages

# 4. Inicie a plataforma
streamlit run app_docking.py
    """, language="bash")
    ...
    if LIBS_INSTALADAS:
        ...
        st.error("🚨 **Atenção:** Módulos fundamentais ausentes. Por favor, ative o ambiente virtual e realize a instalação no terminal seguindo as instruções acima.")
        st.stop()
            ...
            if st.button("Processar, Otimizar e Converter para PDBQT", type="primary"):
                ...
                with st.spinner("Particionando, minimizando em 3D e convertendo para PDBQT (Pode demorar)..."):
                    ...
                    if qtd_gerados > 0:
                        ...
                        if total_falha > 0:
                            st.warning(f"⚠️ Atenção: {total_falha} molécula(s) descartadas devido a erros químicos.")
...
with tab_vina:
    ...
    with col_conf1:
        ...
        if st.session_state.vs_mode:
            st.info("🔹 Modo Triagem Virtual Ativado: O arquivo de configuração não exige o nome do ligante, os arquivos serão processados em lote.")
        else:
    ...
    if st.session_state.vs_mode:
        ...
        if not st.session_state.get('vs_results_dir') or not os.path.exists(st.session_state.vs_results_dir):
            ...
            with col_vis1:
                ...
                # Lista todas as replicatas de todos os ligantes para o usuário escolher o que quer ver/converter
                ...
                if st.button("Sintetizar Ligante Selecionado"):
                    ...
                    with st.spinner("Fundindo matriz..."):
                        ...
                        if os.path.exists("melhor_pose.pdb"):
                            ...
                            # Nome do arquivo PDB resultante baseado na escolha
